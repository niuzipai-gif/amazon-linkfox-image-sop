...
- 画幅：默认只有 4 张 A+ 为 `W 9:H 6`；其余普通图和独立尺寸图为 `1:1`，除非本次任务明确指定其他比例。
- 品牌基因：页面出现“智能品牌基因”时，必须在界面实际选择颜色、字体和整体风格并记录；把这些内容写进提示词不算完成。
...
- 上传参考图先等待 `filechooser`，再用 `chooser.setFiles(...)` 提交；按钮不在视口内先滚动配置面板。若 LinkFox 对具体文件提示超限并要求压缩，核对文件名后直接点击“确定”，再用实际缩略图数量和“主图”标记核验，不把 loading 当成功。
- 上传失败时保留已填写页面和已上传图片，不清空重建页面，不重复同一点击；把上传前后状态、压缩确认和下一状态判据写入任务状态。
