...
$hardRules = [ordered]@{
    ...
    'upload-filechooser' = '(?i)(filechooser|file\s+chooser)'
    'upload-set-files' = '(?i)(setFiles|set\s+files)'
    'upload-compression-confirm' = '(?i)(压缩.{0,60}(确定|确认)|compression.{0,80}(confirm|确定))'
    'upload-verification' = '(?i)(缩略图|thumbnails?).{0,80}(数量|count|核验|verify)|(?i)(数量|count|核验|verify).{0,80}(缩略图|thumbnails?)|参考图数量.{0,80}(回读|verify)|主图.{0,50}(标记|marker)|main[- ]image.{0,50}marker'
    'upload-preserve-page' = '(?i)(保留.{0,40}(页面|配置|已上传)|preserve.{0,40}(page|configuration|uploaded))'
